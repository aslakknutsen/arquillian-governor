...
public class GithubGovernorClient implements GovernorClient<Github, GithubGovernorStrategy> 
{
    ...
    private String getReopenMessage(TestResult testResult, After event)
    {
        Validate.notNull(githubGovernorConfiguration, "Github Governor configuration must be set.");

        return String.format(githubGovernorConfiguration.getReopenMessage(), githubGovernorConfiguration.getUsername(), event.getTestClass().getJavaClass(), event.getTestMethod().getName(), testResult.getThrowable().getMessage());
    }

    @Override
    public void reopen(String issueId, After event, TestResult testResult) {
        Validate.notNull(githubClient, "Github REST client must be specified.");
        ...
        try
        {
            ...
            comment = this.issueService.createComment(this.githubGovernorConfiguration.getRepositoryUser(), this.githubGovernorConfiguration.getRepository(), issueId, getReopenMessage(testResult, event));
            this.issueService.editIssue(this.githubGovernorConfiguration.getRepositoryUser(), this.githubGovernorConfiguration.getRepository(), issue);
        } catch (Exception e)
        {
            if (comment != null) {
